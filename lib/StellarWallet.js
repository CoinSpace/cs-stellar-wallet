...
export default class StellarWallet extends CsWallet {
  ...
  #transformTx(tx) {
    ...
    return new StellarTransaction({
      status: tx.status ? StellarTransaction.STATUS_SUCCESS : StellarTransaction.STATUS_FAILED,
      id: tx.id,
      to,
      from: tx.from,
  ...
  async estimateImport({ privateKey }) {
    ...
    const { sendable, fee } = await this.#prepareImport(privateKey);
    const ledger = await this.#getLedger();
    if (this.#isActive === false && sendable < ledger.minReserve) {
      throw new errors.MinimumReserveDestinationError(new Amount(ledger.minReserve + fee, this.crypto.decimals));
    }
    if (sendable < this.#dustThreshold) {
      throw new errors.SmallAmountError(new Amount(this.#dustThreshold + fee, this.crypto.decimals));
    }
